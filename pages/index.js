    ...
    const items = await Promise.all(
      data.map(async (i) => {
        ...
        let item = {
          ...
          ownership: meta.data.ownership,
          status: meta.data.status,
          warranty: meta.data.warranty,
        };
        return item;
      })
    );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4">
          {nfts.map((nft, i) => (
            <div key={i} className="border shadow rounded-xl overflow-hidden">
              <img src={nft.image} />
              <div className="p-4">
                <p style={{ height: "30px" }} className="text-xl">
                  Name: {nft.name}
                </p>
                ...
                <div style={{ overflow: "hidden" }}>
                  <p className="">Ownership: {nft.ownership}</p>
                </div>
                <div style={{ overflow: "hidden" }}>
                  <p className="">Status: {nft.status}</p>
                </div>
                <div style={{ overflow: "hidden" }}>
